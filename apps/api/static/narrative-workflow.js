const narrativeAuthoring = (() => {
  ...
  let executions = {};
  function executionPayload(stage) {
    if (modes[stage] !== "ai" || !executions[stage]) return {};
    const { task_id, task_version, input_fingerprint, model } = executions[stage];
    return { task_id, task_version, input_fingerprint, model };
  }
  ...
  async function imagineDirector() { const result = await request("/api/narrative/series/director/generate", json("POST", { source_text: value("director-concept"), prompt: value("director-prompt"), model: model() })); fillDirector(result.candidate); modes.director = "ai"; executions.director = result.execution; notify(t("messages.directorCandidate")); }
  async function saveDirector() { workflow = await request("/api/narrative/series/director", json("PUT", { content: directorContent(), mode: modes.director, prompt: value("director-prompt"), model: model(), ...executionPayload("director") })); modes.director = "manual"; refreshSeries(); }
  async function imagineScreenwriter() { const result = await request("/api/narrative/series/screenwriter/generate", json("POST", { prompt: value("screenwriter-prompt"), model: model() })); fillScreenwriter(result.candidate); modes.screenwriter = "ai"; executions.screenwriter = result.execution; notify(t("messages.screenwriterCandidate")); }
  async function saveScreenwriter() { workflow = await request("/api/narrative/series/screenwriter", json("PUT", { content: screenwriterContent(), mode: modes.screenwriter, prompt: value("screenwriter-prompt"), model: model(), ...executionPayload("screenwriter") })); modes.screenwriter = "manual"; refreshSeries(); }
  async function imagineValidator() { const result = await request("/api/narrative/series/validator/generate", json("POST", { prompt: value("validator-prompt"), model: model() })); fillValidator(result.candidate); modes.validator = "ai"; executions.validator = result.execution; notify(t("messages.auditReady")); }
  async function saveValidator() { workflow = await request("/api/narrative/series/validator", json("PUT", { content: { verdict: value("validator-verdict"), summary: value("validator-summary"), findings: validatorFindings }, mode: modes.validator, prompt: value("validator-prompt"), model: model(), ...executionPayload("validator") })); modes.validator = "manual"; refreshSeries(); }
  ...
  async function imagineBreakdown() { if (!currentEpisode) throw new Error(t("errors.selectEpisode")); const result = await request(`/api/episodes/${currentEpisode.id}/breakdown/generate`, json("POST", { prompt: value("episode-author-prompt"), model: model() })); breakdownCandidate = result.candidate.shots; modes.breakdown = "ai"; executions.breakdown = result.execution; renderBreakdown(); notify(t("messages.breakdownCandidate")); }
  async function applyBreakdown() { if (!currentEpisode) throw new Error(t("errors.selectEpisode")); const result = await request(`/api/episodes/${currentEpisode.id}/breakdown/apply`, json("POST", { candidate: { shots: collectBreakdown() }, mode: modes.breakdown, prompt: value("episode-author-prompt"), model: model(), ...executionPayload("breakdown") })); modes.breakdown = "manual"; const id = currentEpisode.id; fillEpisode(result.episode); await window.SerreEpisode?.refresh?.(id); dialog.close(); notify(t("messages.shotsCreated", { count: result.shots.length })); }
