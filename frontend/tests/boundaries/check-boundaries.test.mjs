...
test("refuse les imports interdits, les dépendances inversées et les fetch directs et les globals legacy", async () => {
  ...
  assert.deepEqual(violations.map(({ sourceFile, specifier }) => [sourceFile.replaceAll("\\", "/"), specifier]), [
    ...
    ["features/editor/model.ts", "window.SerreStudio"],
...
test("le CLI fait échouer npm check lorsqu'une frontière est violée", () => {
  ...
  assert.match(forbidden.stderr, /5 violation\(s\) de frontière/);
});
