    ...
    return (
      <main className={styles.root}>
        ...
        <section aria-labelledby="setup-review-title">
          ...
          {diagnosis.data.managedPrerequisites.length ? <Card as="section"><h2>{messages.prerequisites}</h2><ul>{diagnosis.data.managedPrerequisites.map((tool) => <li key={`${tool.id}-${tool.version}`}><strong>{tool.id} {tool.version}</strong><dl><dt>{messages.prerequisiteSource}</dt><dd><a href={tool.source} rel="noreferrer" target="_blank">{tool.source}</a></dd><dt>{messages.prerequisiteDestination}</dt><dd>{tool.destination}</dd><dt>{messages.prerequisiteChecksum}</dt><dd><code>{tool.archiveSha256}</code></dd><dt>{messages.licenses}</dt><dd><a href={tool.licenseUrl} rel="noreferrer" target="_blank">{tool.licenseName}</a></dd></dl></li>)}</ul></Card> : null}
