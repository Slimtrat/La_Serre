...
export interface ApiErrorOptions<TValidation = unknown> {
  status: number;
  statusText?: string;
  data?: unknown;
  cause?: unknown;
...
function readableDetail(data: unknown, status: number, statusText?: string): string {
  if (isPayload(data)) {
...
export class ApiError<TValidation = unknown> extends Error {
  ...
  constructor(options: ApiErrorOptions<TValidation>) {
    const detail = readableDetail(options.data, options.status, options.statusText);
    ...
    if (isPayload(options.data)) {
      this.code = typeof options.data.code === "string" ? options.data.code : undefined;
      this.validation = (options.data.validation ?? options.data.errors ??
        (Array.isArray(options.data.detail) ? options.data.detail : undefined)) as
        | TValidation
        | undefined;
    }
  }
